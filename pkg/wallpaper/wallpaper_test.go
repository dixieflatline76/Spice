...
func TestDownloadAllImages(t *testing.T) {
	...
	wp.pipeline = NewPipeline(context.Background(), wp.cfg, wp.store.(*ImageStore), wp.ProcessImageJob, nil, nil)
...
func TestAddQuery_InitializesPage(t *testing.T) {
	...
	wp.pipeline = NewPipeline(context.Background(), cfg, wp.store.(*ImageStore), func(ctx context.Context, job DownloadJob) (resultImg provider.Image, finalErr error) {
