...
// Collection represents the curated collections for National Palace Museum.
...
type CollectionEntry struct {
	...
	IDs          []int  `json:"ids,omitempty"`           // For "curated" type (cid)
	SearchParams string `json:"search_params,omitempty"` // For "search" type
}

const (
	remoteURL     = "https://raw.githubusercontent.com/dixieflatline76/Spice/main/docs/collections/npm.json"
	cacheFileName = "npm_cache.json"
)
...
func InitRemoteCollection(cfg *wallpaper.Config) (*Collection, error) {
	if err := json.Unmarshal(embeddedJSON, &embeddedCollection); err != nil {
		return nil, fmt.Errorf("failed to parse embedded npm.json: %w", err)
	}

	cacheDir := filepath.Join(config.GetWorkingDir(), "cache", "npm")
	...
	go func() {
		...
		if semver.Compare(col.Version, embeddedCollection.Version) >= 0 {
			...
			if err := saveCache(cachePath, col); err != nil {
				log.Printf("NPM: Failed to save cache: %v", err)
			}
		} else {
			log.Printf("NPM: Remote collection (%s) is older than embedded (%s), ignoring remote", col.Version, embeddedCollection.Version)
	...
	case <-c:
		if fetchedCollection != nil {
			log.Printf("NPM: Successfully loaded remote collection (%s, %d entries)", fetchedCollection.Version, len(fetchedCollection.Entries))
			return fetchedCollection, nil
		}
		log.Printf("NPM: Remote fetch failed: %v", <-fetchErr)
	case <-time.After(3 * time.Second):
		log.Printf("NPM: Remote fetch timed out, falling back to cache")
	}

	if cacheCol, err := loadCache(cachePath); err == nil {
		if semver.Compare(cacheCol.Version, embeddedCollection.Version) >= 0 {
			log.Printf("NPM: Loaded cached collection (%s)", cacheCol.Version)
			return cacheCol, nil
		}
	} else {
		log.Printf("NPM: Failed to load cache: %v", err)
	}

	log.Printf("NPM: Using embedded collection (%s)", embeddedCollection.Version)
...
func RefreshRemoteCollection() (*Collection, error) {
	...
	if semver.Compare(col.Version, embeddedCollection.Version) >= 0 {
		...
		if err := saveCache(cachePath, col); err != nil {
			log.Printf("NPM: Failed to save cache during refresh: %v", err)
		}
		return col, nil
	}
	log.Printf("NPM: Remote collection (%s) is not newer than embedded (%s), no update needed", col.Version, embeddedCollection.Version)
	return nil, nil
}
